            ...
            case TEST_INPUT_BATCH: {
                ...
                for (int i = 0; i < test_batch->n_test_cases; i++) {
                    ...
                    switch (test_batch->function_id) {
                        case IS_ZERO: {
                            test_batch->elems[j] = isZero(arg1);
                            break;
                        }
                        case BIT_NOR: {
                            test_batch->elems[j] = bitNor(arg1, arg2);
                            break;
                        }
                        case DISTINCT_NEGATION: {
                            test_batch->elems[j] = distinctNegation(arg1);
                            break;
                        }
                        case DIVIDE_POWER_2: {
                            test_batch->elems[j] = dividePower2(arg1, arg2);
                            break;
                        }
                        case GET_BYTE: {
                            test_batch->elems[j] = getByte(arg1, arg2);
                            break;
                        }
                        case IS_POSITIVE: {
                            test_batch->elems[j] = isPositive(arg1);
                            break;
                        }
                        case FLOAT_NEGATE: {
                            test_batch->elems[j] = floatNegate(arg1);
                            break;
                        }
                        case IS_LESS_OR_EQUAL: {
                            test_batch->elems[j] = isLessOrEqual(arg1, arg2);
                            break;
                        }
                        case BIT_MASK: {
                            test_batch->elems[j] = bitMask(arg1, arg2);
                            break;
                        }
                        case ADD_OK: {
                            test_batch->elems[j] = addOK(arg1, arg2);
                            break;
                        }
                        case FLOAT_SCALE_64: {
                            test_batch->elems[j] = floatScale64(arg1);
                            break;
                        }
                        case FLOAT_POWER_2: {
                            test_batch->elems[j] = floatPower2(arg1);
                            break;
                        }
                        default: {
                            // Should never happen
