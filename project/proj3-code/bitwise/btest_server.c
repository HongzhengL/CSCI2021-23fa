...
void validate_test_results(int *batch_elems, unsigned batch_size, enum function_id func,
    ...
    for (int i = 0; i < batch_size; i++) {
        ...
        switch (func) {
            case IS_ZERO:
                expected_result = test_isZero(arg1);
                break;
            case DISTINCT_NEGATION:
                expected_result = test_distinctNegation(arg1);
                break;
            case IS_POSITIVE:
                expected_result = test_isPositive(arg1);
                break;
            case FLOAT_NEGATE:
                expected_result = test_floatNegate(arg1);
                break;
            case FLOAT_SCALE_64:
                expected_result = test_floatScale64(arg1);
                break;
            case FLOAT_POWER_2:
                expected_result = test_floatPower2(arg1);
                break;
            case BIT_NOR:
                expected_result = test_bitNor(arg1, arg2);
                break;
            case DIVIDE_POWER_2:
                expected_result = test_dividePower2(arg1, arg2);
                break;
            case GET_BYTE:
                expected_result = test_getByte(arg1, arg2);
                break;
            case IS_LESS_OR_EQUAL:
                expected_result = test_isLessOrEqual(arg1, arg2);
                break;
            case BIT_MASK:
                expected_result = test_bitMask(arg1, arg2);
                break;
            case ADD_OK:
                expected_result = test_addOK(arg1, arg2);
                break;
            default:
                // Should never happen
...
int run_test(shmem_buf_t *sh_buf, enum function_id func, enum test_outcome *previous_outcome,
    ...
    int a1 = 0;
    int a2 = 0;
    while (a1 < test_counts[0] && (num_args == 1 || a2 < test_counts[1])) {
